...
# Check if REGION and STACK_NAME are provided
if [ "$#" -ne 2 ]; then
    echo "Usage: ./deploy.sh <REGION> <STACK_NAME>"
    exit 1
fi
...
do
    ...
    # Build and create the task definition for the container we just pushed
...
EOF
    ...
    if [ "$STATUS" != "ACTIVE" ]; then
        # New service that needs to be deployed because it hasn't been created yet.
...
# Closing statements for all open blocks
